...
// держится в пределах 3.4 процентных пункта, шесть из семи — в пределах 7.1, и лишь один из
// двадцати переходит 9.7. Нужно затем, что после честного перевода в проценты сами числа стали
// небольшими, и без такой мерки непонятно, 6% — это много или мало.
const MARGIN_Q = [[0, 0], [1.58, 0.25], [3.35, 0.5], [5.11, 0.7], [7.13, 0.85], [9.71, 0.95], [12.77, 0.99], [19.5, 1]];
...
export function createEngine(heroList, stats, stratz = null) {
  ...
  // герой на этой позиции играл и выигрывал, флекс — плюс, а не минус. Именно так он и работает
  // в живой игре: необычная роль закрывает то, что соперник не ждёт, и освобождает нормальную
  // роль для сильного героя (выгода от этого считается сама — у того, второго героя).
  // Штраф остаётся там, где доказательств нет вовсе: роль, на которой герой не играет, — это
  ...
  const laneAt = (h, p) => {
    ...
    const w = n / (n + K_LANE);
    // Штраф за чужую роль идёт только на ту долю оценки, которая взята из общей статистики
    // героя. Там, где линия реально сыграна и измерена, он не нужен: измерение уже включает всё,
    // что от этой роли получилось, а второй штраф сверху наказывал бы за неё дважды.
  ...
  function analyze(rad, dire, { posRadiant, posDire } = {}) {
    ...
    // Снизу предел глубокий: состав без керри или с саппортами на ядрах должен читаться как
    // проигранный, а не «чуть хуже». Прежний предел −1.2 упирался сразу у обеих сторон, и
    // разница между ними пропадала: пять саппортов против пяти ядер выходили 45 на 55.
    // Проверочные выборки против этого не возражают — таких составов в реальных матчах нет вовсе.
    // Сверху предел узкий: удачный флекс это плюс, но драфт он сам по себе не выигрывает.
    const posSum = list => clamp(sum(list.map(p => p.pen)), -2.6, 0.25);
