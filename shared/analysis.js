...
export function createEngine(heroList, stats) {
  ...
  function analyze(rad, dire, { posRadiant, posDire } = {}) {
    ...
    // Снизу предел глубокий: состав, где каждый не на своём месте, должен выглядеть проигрышным.
    // Сверху — узкий: удачный флекс это плюс, но он не выигрывает драфт сам по себе, и пятеро
    // «удачно перевёрнутых» героев не должны складываться в перевес на ровном месте.
    const posSum = list => clamp(sum(list.map(p => p.pen)), -1.2, 0.25);
